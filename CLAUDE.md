...
[src/App.tsx](src/App.tsx) is where `useTimerEngine`, `useAmbientSync`, `useCloudSync`, `useWakeLock`, and `useKeyboardShortcuts` are mounted — each exactly once. They are app-wide engines, not reusable component behavior; a second mount means two tick loops or two debounced pushers. `useWakeLock(keepAwake && status === 'running')` keeps the screen alive during a session (mobile blanks at ~30 s); the API is absent in Firefox and old Safari and fails silently on purpose.
...
All color is semantic CSS variables in [src/themes/theme.css](src/themes/theme.css) under `[data-palette="X"][data-theme="Y"]` selectors, bridged to Tailwind via `@theme inline` in [src/index.css](src/index.css) (`bg-surface`, `text-ink`, `border-line`, `text-accent`, …). **No component contains a literal color.** Values are oklch so hue rotation preserves perceived lightness across palettes.
...
One shared `AudioContext` ([src/audio/context.ts](src/audio/context.ts)) for alarms and ambient — browsers cap contexts per page and the user-gesture unlock applies to the whole context. `unlockAudio()` is called on start clicks. Alarms are synthesized (no assets to go missing); ambient are real field recordings in `public/ambient/` (built by `tools/build-ambient.sh`, attributions in `ATTRIBUTION.md`) except white noise, which stays synthesized. Ambient channels are **unmounted** at zero volume rather than left silent; decoded `AudioBuffer`s are cached.
